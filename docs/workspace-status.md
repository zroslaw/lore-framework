...
| S11 | agents on disk with no registered shortcut in any of the four shortcut locations (three workspace-local, plus legacy `~/.codex/skills/`) | info | Name them. Registration is optional — they are always loadable via `boot <agent>` — but a shortcut is the faster entry point. | `register-agent <name>`, or `register-repo <repo>` for all of a repo's agents |
