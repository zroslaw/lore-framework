...
`warnings` in the envelope are about the *scan*, not the workspace. Four kinds are emitted: the
...
`git status` (S1 and S12 suppressed), a symlinked child resolving outside the workspace, and a
declared URL whose derived directory name is unsafe. Print them separately from the findings, one
line each. A suppressed finding is not a clean
...
| S6 | declared repos absent from disk | warn | Name them. Declared repos that are missing mean this workspace is not fully materialized. | `workspace-pull` |
...
| S8 | a top-level repo is not on its default branch | warn | Name the repo, its current branch, and its default. Top-level repos hold production state; branch work belongs in a worktree. | `docs/worktrees.md` — move the work to `.worktrees/<repo>/<slug>/` |
...
| S11 | agents on disk with no registered shortcut on any engine | info | Name them. Registration is optional — they are always loadable via `boot <agent>` — but a shortcut is the faster entry point. | `register-agent <name>`, or `register-repo <repo>` for all of a repo's agents |
| S12 | dirty workspace-root paths outside the managed set | info | List them for visibility only. These are the user's own files; no framework command will touch them, and `workspace-push` deliberately leaves them alone. | none — informational |
| S13 | a declared child is missing, not a git repo, has no origin, or its origin disagrees with the declaration; a declared URL yields an unsafe directory name; a child git repo's own name is unsafe for a `.gitignore` line; or a child symlink escapes the workspace | warn | Name the child and the reason. When `dirname` is null there is no child to name — name the declared URL instead. An origin mismatch is the one to read carefully: `workspace-pull` will refuse to pull that repo until it is resolved. An unsafe *child* directory name cannot be ignored automatically at all: say so, and that renaming the directory is the fix — `workspace-pull` cannot help here | resolve per the `workspace-pull` conflict table (`docs/workspace-pull.md` § Conflict Handling); rename the directory for an unsafe name |
...
| S15 | Codex only — decided **per repo**: an agent repo none of whose agents has a matching `~/.codex/skills/lr-*-agent/` | info | Name the repos in `data.repos`. Codex per-agent shortcuts live in the user's home directory, outside the workspace repo, so they can never arrive by git — these repos' agents are unbootable by shortcut on this machine. Because that directory is user-global and agent names collide across repos by design, a shortcut registered elsewhere for a same-named agent can make a repo look served; treat S15's silence as weak evidence, not proof. | `register-repo <repo>` for each repo named |
