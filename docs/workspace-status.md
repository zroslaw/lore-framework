...
| S8 | a top-level repo is not on its default branch, or is on a detached HEAD | warn | Name the repo, its current branch, and its default. Top-level repos hold production state; branch work belongs in a worktree. When the entry's `detached` is true there is no current branch to name: say the repo is on a detached HEAD, which is reported even when the default branch is unknown — being on no branch is off production state regardless of what the default is, and a commit made there sits on no branch and is lost by the next checkout. | `docs/worktrees.md` — move the work to `.worktrees/<repo>/<slug>/`; for a detached child, `git -C <dirname> checkout <default>` — and when `default` is null, say to check out the repo's own default branch rather than printing a placeholder there is no value for |
