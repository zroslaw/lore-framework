...
1. Look at all files in the domain directory's `.claude/commands/` (i.e., the `.claude/commands/` folder sibling to `lore-framework/`, **not** `~/.claude/commands/`) matching the pattern `lr-*-agent.md` — each file is a registered agent boot command. The filename encodes the agent name (e.g., `lr-lore-architect-agent.md` → agent `lore-architect`).
