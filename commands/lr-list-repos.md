...
1. Look at all files in the domain directory's `.claude/commands/` (i.e., the `.claude/commands/` folder sibling to `lore-framework/`, **not** `~/.claude/commands/`) matching the pattern `lr-*-agent.md` — each file belongs to a registered agent. The boot command file contains the path to the agent's `role.md`, which reveals the repo it lives in.
