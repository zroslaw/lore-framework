...
1. **Discover the agent.** Search all directories in the current working directory for lore agent repos — directories containing a `lore-repo.md` file at the root. Within each, look for `agents/$ARGUMENTS/` containing `role.md`.

2. **If not found**, list all available agents (scan all lore agent repos) and report the error. Stop here.
