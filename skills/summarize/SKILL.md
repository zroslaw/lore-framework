...
`<framework-root>` is the framework root — the directory that contains the `VERSION` file, two levels up from this `skills/summarize/SKILL.md`. Resolve it to an absolute path before using it below.
