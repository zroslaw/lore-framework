...
`<framework-root>` is the framework root — the directory that contains the `VERSION` file, three levels up from this `skills/cursor/lr-merge/SKILL.md`. Resolve it to an absolute path before using it below.
